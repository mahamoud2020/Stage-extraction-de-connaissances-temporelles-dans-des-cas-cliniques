...
# ************************************************************************************************************
# Dictionnaire de traduction en français pour DataTables (Indépendant d'Internet)
lang_fr <- list(
...
xml_reconstruit_list <- lapply(1:nrow(chaines_uniques), function(i) {
  ...
  # Découpage avec le séparateur 
  heads <- unlist(strsplit(as.character(current_chaine), "\\s*\\|\\s*"))
...
# Définition du thème personnalisé pour retrouver les couleurs CSS
my_theme <- bs_theme(
  bootswatch = "flatly", 
  primary = "#2C3E50",    
  secondary = "#18BC9C",
...
ui <- fluidPage(
  ...
  div(style = "text-align: center; margin-top: 20px; margin-bottom: 30px;",
      tags$h2(tags$b("Analyse de Séquence appliquée aux chaînes de coréférences"), style="color: #2C3E50;")
  ),
  
  # La zone contenant tous les onglets
  navset_card_tab(
    ...
    nav_panel(
      ...
      navset_tab(
        nav_panel(
          ...
          card(
            card_header(class = "bg-primary text-white", tags$strong("Jeu de données : sequences_coreferences_francais.csv")),
            p(style = "color:#555; font-size:1.05em; margin-top: 15px;",
        ...
        nav_panel(
          ...
          layout_sidebar(
            ...
            card(
              full_screen = TRUE,
              card_header(class = "bg-primary text-white", tags$strong(textOutput("vue_plot_title"))),
              plotOutput("vue_plot_out", height = "520px"),
              div(style="text-align:center; margin-top:15px;", 
                  downloadButton("dl_vue_pdf", "PDF"), 
                  downloadButton("dl_vue_png", "PNG"),
                  downloadButton("dl_vue_jpg", "JPG")
              )
            )
          )
        )
    ...
    nav_panel(
      ...
      layout_sidebar(
        sidebar = sidebar(
          width = 300, bg = "#EDF7F6",
          h5(tags$b("Configuration"), style = "color: #16A085;"),
          sliderInput("nb_clusters", label = "Nombre de classes (k) :", min = 2, max = 10, value = 4, step = 1),
          hr(),
          h6(tags$b("Tailles"), style = "color: #2C3E50;"),
          tableOutput("cluster_sizes"),
          hr(),
          h6(tags$b("Type de Graphique TraMineR"), style = "color: #2C3E50;"),
        ...
        navset_tab(
          nav_panel(
            ...
            layout_columns(
              col_widths = c(6, 6),
              card(full_screen = TRUE, card_header(class="bg-primary text-white", tags$b("Dendrogramme Ward ")), 
                   plotOutput("dendro_simple", height = "420px"),
                   div(style="text-align:center; margin-top:10px;", downloadButton("dl_dendro_simple_pdf", "PDF"), downloadButton("dl_dendro_simple_png", "PNG"), downloadButton("dl_dendro_simple_jpg", "JPG"))
              ),
              card(full_screen = TRUE, card_header(class="bg-secondary text-white", tags$b("Dendrogramme avec Heatmap")), 
                   plotOutput("dendro_couleur", height = "420px"),
                   div(style="text-align:center; margin-top:10px;", downloadButton("dl_dendro_col_pdf", "PDF"), downloadButton("dl_dendro_col_png", "PNG"), downloadButton("dl_dendro_col_jpg", "JPG"))
              )
            )
          ),
          nav_panel(
            ...
            layout_columns(
              col_widths = c(6, 6),
              card(full_screen = TRUE, card_header(class="bg-primary text-white", tags$b("Sauts d'inertie simples")), 
                   plotOutput("inertie_simple", height = "380px"),
                   div(style="text-align:center; margin-top:10px;", downloadButton("dl_inertie_pdf", "PDF"), downloadButton("dl_inertie_png", "PNG"), downloadButton("dl_inertie_jpg", "JPG"))
              ),
              card(full_screen = TRUE, card_header(class="bg-secondary text-white", tags$b("Coupures suggérées (k optimal)")), 
                   plotOutput("inertie_couleur", height = "380px"),
                   div(style="text-align:center; margin-top:10px;", downloadButton("dl_inertie_col_pdf", "PDF"), downloadButton("dl_inertie_col_png", "PNG"), downloadButton("dl_inertie_col_jpg", "JPG"))
              )
          ...
          nav_panel(
            ...
            card_header(class="bg-info text-white", tags$b(textOutput("cluster_card_title"))),
            p(style = "font-size:0.85em; color:#666;", textOutput("cluster_plot_desc")),
            plotOutput("cluster_plot_out", height = "700px"),
            div(style="text-align:center; margin-top:15px;", 
                downloadButton("dl_traminer_pdf", "Exporter en PDF"), 
                downloadButton("dl_traminer_png", "Exporter en PNG"),
                downloadButton("dl_traminer_jpg", "Exporter en JPG")
            )
          )
        )
      )
...
server <- function(input, output, session) {
  ...
  # Fonction d'affichage du premier graphique
  plot_vue <- function() {
  ...
  output$vue_plot_out <- renderPlot({ plot_vue() }, res = 100)
  
  # Exports Graphique Onglet 1
  output$dl_vue_pdf <- downloadHandler(filename = "visualisation_corpus.pdf", content = function(file) { pdf(file, width=12, height=8); plot_vue(); dev.off() })
  output$dl_vue_png <- downloadHandler(filename = "visualisation_corpus.png", content = function(file) { png(file, width=1200, height=800, res=100); plot_vue(); dev.off() })
  output$dl_vue_jpg <- downloadHandler(filename = "visualisation_corpus.jpg", content = function(file) { jpeg(file, width=1200, height=800, res=100); plot_vue(); dev.off() })
  
  
  # Graphiques d'analyse (Onglet 2)
  ...
  # Fonction de plot TraMineR
  plot_traminer <- function() {
    d <- clustering_react()
  ...
  output$cluster_plot_out <- renderPlot({ plot_traminer() }, res = 100, height = 700) 
  
  # Fonctions d'exports pour Onglet 2
  plot_dendro_simple <- function() { plot(as.dendrogram(arbre_ward), main = "Arbre de Ward", leaflab = "none", xlab = "Séquences", ylab = "Inertie") }
  plot_dendro_col <- function() { seq_heatmap_custom(coref_chaines.seq, arbre_ward) }
  plot_inertie_simple <- function() { plot(sort(arbre_ward$height, decreasing = TRUE)[1:15], type = 's', xlab = "k", ylab = "Inertie", col = "blue", lwd = 2) }
  plot_inertie_col <- function() {
    plot(hauteurs, type = 's', xlab = "Nombre de clusters", ylab = "Inertie", col = "steelblue", lwd = 2.5)
    abline(v = grands_sauts_idx, lty = 2, col = adjustcolor(couleurs_sauts, alpha.f = 0.4), lwd = 1.5)
    points(grands_sauts_idx, hauteurs[grands_sauts_idx], pch = 21, cex = 2.5, bg = couleurs_sauts, col = "white")
    text(grands_sauts_idx, hauteurs[grands_sauts_idx], labels = paste0("k = ", grands_sauts_idx), pos = 1, col = couleurs_sauts, font = 2)
  ...
  output$dl_dendro_simple_pdf <- downloadHandler(filename = "dendrogramme.pdf", content = function(file) { pdf(file, width=10, height=7); plot_dendro_simple(); dev.off() })
  output$dl_dendro_simple_png <- downloadHandler(filename = "dendrogramme.png", content = function(file) { png(file, width=1000, height=700, res=100); plot_dendro_simple(); dev.off() })
  output$dl_dendro_simple_jpg <- downloadHandler(filename = "dendrogramme.jpg", content = function(file) { jpeg(file, width=1000, height=700, res=100); plot_dendro_simple(); dev.off() })
  
  output$dl_dendro_col_pdf <- downloadHandler(filename = "dendrogramme_heatmap.pdf", content = function(file) { pdf(file, width=10, height=7); plot_dendro_col(); dev.off() })
  output$dl_dendro_col_png <- downloadHandler(filename = "dendrogramme_heatmap.png", content = function(file) { png(file, width=1000, height=700, res=100); plot_dendro_col(); dev.off() })
  output$dl_dendro_col_jpg <- downloadHandler(filename = "dendrogramme_heatmap.jpg", content = function(file) { jpeg(file, width=1000, height=700, res=100); plot_dendro_col(); dev.off() })
  
  output$dl_inertie_pdf <- downloadHandler(filename = "inertie.pdf", content = function(file) { pdf(file, width=8, height=6); plot_inertie_simple(); dev.off() })
  output$dl_inertie_png <- downloadHandler(filename = "inertie.png", content = function(file) { png(file, width=800, height=600, res=100); plot_inertie_simple(); dev.off() })
  output$dl_inertie_jpg <- downloadHandler(filename = "inertie.jpg", content = function(file) { jpeg(file, width=800, height=600, res=100); plot_inertie_simple(); dev.off() })
  
  output$dl_inertie_col_pdf <- downloadHandler(filename = "inertie_optimale.pdf", content = function(file) { pdf(file, width=8, height=6); plot_inertie_col(); dev.off() })
  output$dl_inertie_col_png <- downloadHandler(filename = "inertie_optimale.png", content = function(file) { png(file, width=800, height=600, res=100); plot_inertie_col(); dev.off() })
  output$dl_inertie_col_jpg <- downloadHandler(filename = "inertie_optimale.jpg", content = function(file) { jpeg(file, width=800, height=600, res=100); plot_inertie_col(); dev.off() })
  
  output$dl_traminer_pdf <- downloadHandler(filename = "graphique_traminer.pdf", content = function(file) { pdf(file, width=12, height=8); plot_traminer(); dev.off() })
  output$dl_traminer_png <- downloadHandler(filename = "graphique_traminer.png", content = function(file) { png(file, width=1200, height=800, res=100); plot_traminer(); dev.off() })
  output$dl_traminer_jpg <- downloadHandler(filename = "graphique_traminer.jpg", content = function(file) { jpeg(file, width=1200, height=800, res=100); plot_traminer(); dev.off() })
  ...
  # Tableau 3 : héritage des annotations (Avec TOUTES les colonnes)
