...
CSV_RESULTATS = "data/resultats_coreferences_english.csv" 
...
def appeler_udpipe2(texte_brut):
    # Modèle anglais
    params = {"model": "english-ewt-ud-2.12-230717", "tokenizer": "", "tagger": "", "parser": ""}
    try:
...
def main():
    print("Étape 1 : Conversion via UDPipe 2 API (avec le modèle Anglais)")
    ...
    for file_path in glob.glob(os.path.join(INPUT_DIR, "*.xml")):
        ...
        if res:
            # S'assure que le dossier d'entrée existe
